...
let CORE_KNOWLEDGE = {};
try {
  CORE_KNOWLEDGE = JSON.parse(
    fs.readFileSync(path.join(__dirname, "knowledge.json"), "utf8")
  );
} catch (error) {
  console.warn("knowledge.json could not be loaded:", error.message);
}
...
function getKnowledgeContext(message) {
  ...
  const keywords = {
    india_gk: ["भारत", "india", "राष्ट्रीय", "राजधानी", "राज्य", "उत्तर प्रदेश", "यूपी", "देश"],
    indian_polity: ["संविधान", "अनुच्छेद", "संसद", "लोकसभा", "राज्यसभा", "राष्ट्रपति", "मौलिक अधिकार", "राजनीति"],
    history: ["इतिहास", "1857", "कांग्रेस", "जलियांवाला", "असहयोग", "सविनय", "भारत छोड़ो", "मौर्य", "हड़प्पा"],
    geography: ["भूगोल", "नदी", "पर्वत", "महासागर", "महाद्वीप", "मानसून", "अक्षांश", "देशांतर", "कर्क रेखा"],
    science: ["विज्ञान", "physics", "भौतिक", "chemistry", "रसायन", "biology", "जीव", "विटामिन", "बल", "ऊर्जा", "डीएनए", "dna"],
    computer_gk: ["कंप्यूटर", "computer", "html", "css", "javascript", "cpu", "ram", "rom", "url", "http", "binary"],
    general_static_gk: ["सामान्य ज्ञान", "gk", "general knowledge", "संयुक्त राष्ट्र", "un", "सौरमंडल", "ग्रह", "ओलंपिक", "दिवस"],
    exam_basics: ["gk gs", "जीके", "जीएस", "gs", "competitive", "प्रतियोगी", "exam", "परीक्षा", "reasoning", "aptitude"]
  };

  for (const [group, keys] of Object.entries(keywords)) {
    if (keys.some(k => text.includes(k)) && Array.isArray(CORE_KNOWLEDGE[group])) {
      groups.push(group);
    }
  }

  // If the user explicitly asks for GK/GS, provide all core categories.
  if (/(^|\\s)(gk|gs)(\\s|$)/.test(text) || text.includes("सामान्य ज्ञान") || text.includes("general knowledge")) {
    for (const group of ["india_gk","indian_polity","history","geography","science","computer_gk","general_static_gk"]) {
      if (Array.isArray(CORE_KNOWLEDGE[group]) && !groups.includes(group)) groups.push(group);
    }
  }

  const selected = groups.flatMap(group => CORE_KNOWLEDGE[group] || []);
  if (!selected.length) return "";

  // Keep prompt size controlled while still giving the model useful factual grounding.
  return selected.slice(0, 80).map(item => {
    if (item.q && item.a) return `Q: ${item.q}\\nA: ${item.a}`;
    if (item.topic && item.facts) return `Topic: ${item.topic}\\n- ${item.facts.join("\\n- ")}`;
    return "";
  }).filter(Boolean).join("\\n");
}
...
- A built-in knowledge file named knowledge.json contains foundational GK/GS facts. When the supplied knowledge is relevant, use it as a factual reference.
- Treat changing facts (current affairs, current office holders, latest statistics, rankings, laws, prices, etc.) as requiring fresh verification; do not treat the static knowledge file as current for those topics.
${knowledgeContext ? `

Built-in GK/GS knowledge relevant to this question:
${knowledgeContext}` : ""}
