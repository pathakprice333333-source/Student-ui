...
async function sendMessage() {
  ...
  if (attachedFile?.processing) {
    attachmentStatus.textContent = "Photo/file abhi process ho rahi hai. 2–3 seconds baad try karo.";
    return;
  }
  ...
  // Instant fixed greeting is used only when there is NO attachment.
  // With a photo/file attached, every command must reach the AI with the attachment.
  if (isHelloMessage(text) && !attachedFile) {
...
fileInput.addEventListener("change", async () => {
  ...
  try {
    ...
    attachedFile = { ...data.file, processing: Boolean(data.processing) };
    ...
    attachedFile = null;
    if (attachmentPreviewUrl) { URL.revokeObjectURL(attachmentPreviewUrl); attachmentPreviewUrl = ""; }
